...
from ._pdf_util import PdfExtractor
from typing import *
...
def find_match_page(
    ...
    browser = PreprocessorBrowser(doc_dir=doc_dir)
    contents = browser.get_pure_contents(pdf_name=pdf_name)
    pure_text = []
    for content in contents:
        pure_text.extend(content.text_lines)

    if len(pure_text) == 0:
        logger.info("No pure text found for {}".format(pdf_name))
        return None, None
    ...
    for i in range(len(text_lines)):
        ...
        if num_match < min_match_number:
            if DEBUG:
                print(
                    "Filter as not enough post keywords {}<{}".format(
                        num_match, min_match_number
                    )
                )
...
def extract_table_for_rows(
    ...
    pdf_name = metadata["key"]
    match_page_id, max_page_id = find_match_page(
        pdf_name,
    ...
    if match_page_id is None:
        logger.debug(f"No match page found for {pdf_name}")
        return []

    pdf_path = metadata["pdf_path"]
    extractor = PdfExtractor(path=pdf_path)
    ...
    try:
        ...
        logger.error(e, pdf_name, "parse error")
        near_tables = []
...
def tables_to_file(tables, file_path, skip_existing=True):
    if len(tables) == 0:
        logger.debug(f"No tables found for {file_path}")
        return
    text = ""
    for table in tables:
        text += "page|{}\n".format(table.page)
...
def extract_basic_info(metadata: Dict, doc_dir: str, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "basic_info.txt")
    ...
    logger.info(f"extracting basic info for {metadata['key']}")
    bi_tables = extract_table_for_rows(
        metadata=metadata,
        max_continuous_lines=int(len(row_keywords) * 1.5),
    ...
    tables_to_file(filtered_tables, out_path)


def extract_employee_info(metadata: Dict, doc_dir: str, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "employee_info.txt")
    if os.path.exists(out_path) and skip_existing:
        return
    row_keywords = [
        "在职员工",
    ...
    ei_tables = extract_table_for_rows(
        metadata=metadata,
        max_continuous_lines=int(1.5 * len(row_keywords)),
        min_match_number=0.3 * len(row_keywords),
        required_line_keywords=[
            "员工情况",
            "员工的数量",
...
def extract_cbs_info(metadata, doc_dir, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "cbs_info.txt")
    ...
    func = partial(
        extract_table_for_rows,
        metadata=metadata,
        max_continuous_lines=int(1.5 * len(row_names)),
    ...
    if len(cbs_tables) == 0:
        ...
        cbs_tables = func(
            max_continuous_lines=int(2 * len(row_names)),
            min_match_number=0.2 * len(row_names),
    ...
    filtered_tables = remove_overlap_tables(
        filtered_tables,
...
def extract_cscf_info(metadata, doc_dir, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "cscf_info.txt")
    if skip_existing and os.path.exists(out_path):
        return
    ...
    filtered_tables = remove_overlap_tables(
        cscf_tables, valid_overlap_words=["净额", "人民币"]
    )
...
def extract_cis_info(metadata, doc_dir, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "cis_info.txt")
    if skip_existing and os.path.exists(out_path):
        return
    row_keywords = [
        "营业总收入",
    ...
    cis_tables = extract_table_for_rows(
        metadata=metadata,
        max_continuous_lines=int(1.5 * len(row_keywords)),
        min_match_number=0.3 * len(row_keywords),
    ...
    # 删除母公司XX
    filtered_tables = remove_overlap_tables(
        filtered_tables, valid_overlap_words=["利息收入", "项目"]
    )
    # 删除没过滤掉的
    filtered_tables = remove_tables_same_page_by_keywords(filtered_tables, row_keywords)
    # 删除跨页的
...
def extract_dev_info(metadata, doc_dir, skip_existing=True):
    out_path = os.path.join(doc_dir, metadata["key"], "dev_info.txt")
    if skip_existing and os.path.exists(out_path):
        return
    row_keywords = ["研发人员数量", "研发人员的数量"]

    dev_tables = extract_table_for_rows(
        metadata=metadata,
        max_continuous_lines=int(1.5 * len(row_keywords)),
...
def extract_metadata(metadata, doc_dir, skip_existing=True):
    file_path = os.path.join(doc_dir, metadata["key"], "metadata.json")
...
def merge_all_table_infos(doc_dir: str):
    for root, dirs, files in os.walk(doc_dir):
        for dir in dirs:
            if not re.match(dir_pattern, dir):
                continue
            p = os.path.join(doc_dir, dir)
            md = os.path.join(p, "metadata.json")
            bi = os.path.join(p, "basic_info.txt")
            ei = os.path.join(p, "employee_info.txt")
            cbs = os.path.join(p, "cbs_info.txt")
            cscf = os.path.join(p, "cscf_info.txt")
            cis = os.path.join(p, "cis_info.txt")
            dev = os.path.join(p, "dev_info.txt")
            merged = os.path.join(p, "merged.json")

            with open(merged, "w", encoding="utf-8") as f:
                json.dump(
                    {
                        "metadata": json.load(open(md, "r", encoding="utf-8")),
                        "basic_info": table_as_list(bi),
                        "employee_info": table_as_list(ei),
                        "cbs_info": table_as_list(cbs),
                        "cscf_info": table_as_list(cscf),
                        "cis_info": table_as_list(cis),
                        "dev_info": table_as_list(dev),
                    },
                    f,
                    ensure_ascii=False,
                    indent=4,
...
def extract_pure_content(key, pdf_path, save_dir, xpdf_path, skip_existing=True):
    # setup xpdf
    os.chdir(xpdf_path)
    ...
    if os.path.exists(save_path):
        if skip_existing:
            return
        os.remove(save_path)
    PdfExtractor(path=pdf_path, xpdf_path=xpdf_path).extract_pure_content_and_save(
        save_path
    )


def table_as_list(path) -> str:
